...
import vexana.modules.sql.welcome_sql as sql
from vexana import (
    DEV_USERS,
    OWNER_ID,
    DRAGONS,
...
from vexana.modules.helper_funcs.string_handling import (
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    f"Welcome Axel to {html.escape(chat.title)} group mamber plz note my Owner Just arrive here.", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Axel my Owner just joined the chat"
                )
                continue
            ...
            if new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Whoa! The Member of VexanaFanClub just joined!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DRAGONS:
                update.effective_message.reply_text(
                    "Huh! An sudo User  just joined! Stay Alert!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DEMONS:
                update.effective_message.reply_text(
                    "Huh! Someone with VexanaFanClub membership just joined!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in WOLVES:
                update.effective_message.reply_text(
                    "Oof! A Supporter  Users just joined!", reply_to_message_id=reply
                )
                continue
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Sorry to see you leave Will Miss You a lot Axel (OWNER):(", reply_to_message_id=reply
                )
                return
            ...
            if left_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "See you later at the VexanaFanClub!",
                    reply_to_message_id=reply,
                )
                return
