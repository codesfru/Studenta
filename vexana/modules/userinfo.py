...
from vexana.modules.sql.afk_sql import is_afk, set_afk
...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text("<code>Taking a small Look in database...</code>", parse_mode=ParseMode.HTML)
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe access level of this person is 'OWNER'."
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis user is member of 'VexanaFanClub'."
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThe access level of this person is 'Vexana-Lover'."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThe access level of this person is 'Vexana-Supporter'."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThe access level of this person is 'Vexana-Approval."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThe Disaster level of this person is 'Vexana-MOD'."
        disaster_level_present = True
    elif user.id == 5002663814:
         text += "\n\nOwner Of VexanaFAnClub.@. The access level of this person is 'OWNER'."
         disaster_level_present = True
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/OnePunchUpdates/155"),
                            InlineKeyboardButton(
                                "Mine Info", url="https://t.me/Vexana_Updates/3")
                        ],
                    ]
                ),
                parse_mode=ParseMode.HTML,
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>╔═━「 📊 Current stats of Vexana:- Made with Love by Axel Running Of Yarn:-13.7」</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)
    result += "\n<b>╘═━「 Powered By VexanaFanCLub 」</b>"
    update.effective_message.reply_text(
        result,
        parse_mode=ParseMode.HTML, 
        disable_web_page_preview=True
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/OnePunchUpdates/155)
"""
